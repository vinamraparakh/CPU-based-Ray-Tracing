...
public:
...
#ifdef BG
        generateBG("/Users/anirudh/Desktop/OpenGLProjects/RayTracer/Resources/images/envmap6.jpg");
#endif
        GenerateScene();
    }
    ...
    static void GenerateScene() {
        ...
        //glass
        scene.Spheres.push_back(Sphere(vec3(-8.5, 2, -18), 2,  vec3(0.0, 0.2, 0.1), 0.8, vec3(0.6, 0.7, 0.8), 125, 1.5));
        //light blue
        scene.Spheres.push_back(Sphere(vec3(-11, 3, -28), 1, vec3(0.4, 0.2, 0.1), 0.0, vec3(0.3, 0.3, 0.6), 40, 1.0));
        //red
        scene.Spheres.push_back(Sphere(vec3(-14, 3, -28), 1, vec3(0.4, 0.2, 0.1), 0.0, vec3(0.8, 0.1, 0.2), 40, 1.0));
    ...
    inline static float gammaCorrection = 0.7f;
...
private:
    ...
    static float reflectance(float cos, float ri) {
        float r = pow((1.0f - ri) / (1.0f + ri), 2);
        return r + (1.0f - r) * pow((1.0f - cos), 5);
    }
    ...
    static vec3 castRay(vec3 origin, vec3& dir, std::vector<Sphere>& spheres, std::vector<Light>& lights, int pixel, int depth = 0) {
        ...
        if(depth < REFLECTION_COUNT && sceneIntersects(origin, dir, spheres, point, normal, sphere)) {
            ...
            if(sphere.Specular > 100.0f || reflectance(abs(dir * normal), sphere.Refractive_Index) > 0.6f) {
                vec3 reflectDir = reflect(dir, normal).normalize();
                vec3 reflectOrigin = reflectDir * normal < 0 ? point - normal * 1e-3 : point + normal * 1e-3;
                reflectColor = castRay(reflectOrigin, reflectDir, spheres, lights, pixel, depth + 1);
            }
...
#ifdef BG
        int x = ((int)(((atan2(dir.z, dir.x) / (2.0f * 3.14f)) + 0.5f) * ENV_WIDTH + (int)((2.3f * ENV_WIDTH + origin.x * CLIPPING_DISTANCE) / ( 2.0f * 3.14f))));
        x = (x % ENV_WIDTH + ENV_WIDTH) % ENV_WIDTH;
    ...
    static void DrawToFile(std::vector<Sphere>& spheres, std::vector<Light>& lights, Camera& camera) {
        ...
        for (int i = 0; i < HEIGHT * WIDTH; i++) {
            for (int j = 0; j < 3; j++) {
                texture << (char)(255 * std::max(0.f, std::min(1.f, pow(framebuffer[i][j], gammaCorrection))));
            }
        }
        texture.close();
    }
